...
@app_blueprint.route('/edit/<int:id>', methods = ['POST'])
@login_required
def edit(id):
    marker_to_edit = Markers.query.filter_by(id=id).first()
    marker_to_edit.title= request.form['name']
    marker_to_edit.text = request.form['description']
    try: 
     db.session.merge(marker_to_edit)
     db.session.commit()
    except Exception as e: 
     print(e)
     return "False"
